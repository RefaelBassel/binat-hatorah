...
2. **דשבורד מורה — "מסע הרפלקציה של הכיתה":** (א) צ'יפים: סה"כ רפלקציות,
   ...
   באותו רכיב גרף (ה-tooltip מציג "N רפלקציות ביום זה"); (ג) **רשת "המסע של
   כל תלמיד/ה"** — כרטיס לכל תלמיד/ה עם מיני-גרף (וריאנט `mini`), מספר
   הרפלקציות, ותקציר האחרונה; ממוין לפי פעילות אחרונה; מי שטרם שיתפ/ה מוצג
   בכרטיס ריק מעודד. רענון כל 45 שניות + פוקוס. רשימת "הרפלקציות האחרונות —
   במילים שלהם" נשארת מתחת (הטקסט החופשי חשוב למורה).
