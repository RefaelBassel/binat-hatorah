...
export async function POST(req: Request) {
  ...
  const msg = await client.messages.create({
    model: CLAUDE_MODEL,
    max_tokens: 1200,
    system: `את/ה עוזר/ת הערכה למורה באתר "בינת התורה" (תנ"ך, כיתה י, תיכון שחרית).
הערכי את הגשת התלמידה בעברית: ציון 0-100 והערכה מילולית חמה, מפורטת ובונה (מה חוזק, מה לשפר, דוגמה אחת קונקרטית).
שימי לב במיוחד ל: הבנת הפשט, איכות פירוק הטיעון (טענה/נימוק/ביסוס), עומק שאלות שהיא שאלה, ואיכות הסימונים (מילה מנחה, מילים קשות).
זו הצעה בלבד — המורה תערוך ותאשר. החזירי JSON בלבד במבנה: {"score": <מספר>, "feedback": "<טקסט>"}`,
    messages: [
      {
        role: "user",
        content: `שם התלמידה: ${studentName} (פני אליה בשמה — אל תמציאי שם אחר)
